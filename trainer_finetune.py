...
parser.add_argument('--lr', default=0.003, type=float, help='Saved Learning rate')
# parser.add_argument('--lr1', default=0.003, type=float, help='Learning rate')
parser.add_argument('--weight_decay', default=0.01, type=float, help='Weight decay')
# Scheduler
# parser.add_argument('--patience', default=10, type=float, help='Patience for learning rate scheduler')
parser.add_argument('--epochs1', default=300, type=int, help='Saved epochs')
parser.add_argument('--epochs2', default=50, type=int, help='Training epochs')
# Dataloader
...
output_filename = 'argparse_config_finetune.txt'

if not os.path.exists(output_directory):
    print(f"Error: Directory '{output_directory}' does not exist. Exiting.")
    sys.exit(1)  # Exit the program with a non-zero status code
else:
    print(f"Directory '{output_directory}' exists.")
...
def train():
    ...
    for batch_idx, (inputs, targets) in pbar:
        ...
        outputs = torch.cat((outputs[:, :3], outputs[:, 4:5]), dim=1)
        targets = torch.cat((targets[:, :3], targets[:, 4:5]), dim=1)
        ...
        tp = torch.zeros(d_output_test, dtype=torch.float)
        fp = torch.zeros(d_output_test, dtype=torch.float)
        fn = torch.zeros(d_output_test, dtype=torch.float)
        tn = torch.zeros(d_output_test, dtype=torch.float)
        for c in range(d_output_test):
        ...
        total += targets.size(0) * d_output_test
...
def eval(epoch, dataloader, checkpoint=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in pbar:
            ...
            eval_loss += loss.item()
            predicted = outputs.gt(0.5).long()
            tp = torch.zeros(d_output_test, dtype=torch.float).to(device)
            fp = torch.zeros(d_output_test, dtype=torch.float).to(device)
            fn = torch.zeros(d_output_test, dtype=torch.float).to(device)
            tn = torch.zeros(d_output_test, dtype=torch.float).to(device)
            for c in range(d_output_test):
                tp[c] = ((predicted[:, c] == 1) & (targets[:, c] == 1)).sum().item()
                fp[c] = ((predicted[:, c] == 1) & (targets[:, c] == 0)).sum().item()
                fn[c] = ((predicted[:, c] == 0) & (targets[:, c] == 1)).sum().item()
...
# Specify conditions for trainable layers
def is_trainable_layer(name):
    # Keep layers in 'encoder', 'decoder', or specific sparsity_mask layers trainable
    return (
        "encoder" in name or
        "decoder" in name or
        name in {
...
# Freeze all layers not matching the condition
for name, param in model.named_parameters():
    param.requires_grad = is_trainable_layer(name)
...
print("\nFrozen Layers:")
frozen_layers = []
frozen_weights_count = 0

for name, param in model.named_parameters():
    if not param.requires_grad:
        frozen_layers.append(name)
        frozen_weights_count += param.numel()
        print(f"{name} - {param.numel()} weights")

print(f"\nTotal Frozen Layers: {len(frozen_layers)}")
print(f"Total Frozen Weights: {frozen_weights_count}")
...
pbar = tqdm(range(start_epoch, args.epochs2))
for epoch in pbar:
    if epoch == 0:
...
torch.save(model.state_dict(), directory_path + '/brazil_model_' + str(n_layers) + '_' + str(args.lr) + '-' + str(args.epochs1) + '-finetuend' + '.pt')
...
csv_file = directory_path + '/evaluation_metrics_' + str(n_layers) + '_' + str(args.lr) + '-finetuend' + '.csv'
...
# specify the csv file path
csv_file = directory_path + '/training_metrics_' + str(n_layers) + '_' + str(args.lr) + '-finetuend' + '.csv'
